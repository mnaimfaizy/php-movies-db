...
$genre = $_GET['genre'];
$sql = "SELECT * from movies WHERE `genre` LIKE '%".$genre."%' ORDER BY movie_id DESC ";
$paginationlink = "ajax/loadMoviesByGenre.php?page=";	
...
if($db_handle->numRows($sql) < 1) {
	print '<div class="empty-state"><i class="fas fa-theater-masks"></i><h4>No Movies Found</h4><p>Sorry, no movies found for this genre. Please select another genre!</p></div>';
} else { 
	$output = '';
