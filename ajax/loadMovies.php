...
$paginationlink = "ajax/loadMovies.php?page=";	
...
foreach($faq as $movie) {
	...
	$output .= '<a href="single.php?movie_id='.$movie['movie_id'].'" data-toggle="tooltip" data-placement="top" title="'.$movie['movie_title'].'">';
	$output .= '<div class="grid_2">';
	$output .= '<img src="'.$poster.'" width="250" height="229" style="width:250px; height:229px;" class="img-responsive" alt="'.$movie['movie_title'].'"/>';
	$output .= '<div class="caption1">';
	$output .= '<ul class="list_3">';
	$output .= '<li><p style="color: #FFF; background-color:#C00003; padding: 5px; border-radius: 50px;">'.$movie['year'].'</p></li>';
	$output .= '</ul>';
	$output .= '<i class="icon4"> </i>';
	$output .= '<p class="m_3" style="color: #FFFFFF; background-color:#55ACEE;">'.$movie['movie_title'].'</p>';
	$output .= '</div>';
   	$output .= '</div></a>';
	$output .= '</div>';
}
